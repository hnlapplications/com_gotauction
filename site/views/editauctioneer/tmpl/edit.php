...
<form action="<?php echo JRoute::_('index.php?option=com_gotauction&view=editauctioneer&layout=edit&id='.(int)$this->item->id); ?>" method="POST" enctype="multipart/form-data" name="adminForm" id="adminForm" class=form-validate">
	<div class="btn-toolbar">
		<div class="btn-group">
			<button tyoe="button" class="btn btn-primary" onclick="Joomla.submitbutton('editauctioneer.save');">
				<i class="icon-new"></i> <?php echo JText::_('COM_GOTAUCTION_BUTTON_SAVE_AND_CLOSE') ?>
			</button>
		</div>
		<div class="btn-group">
			<button type="button" class="btn btn-primary" onclick="Joomla.submitbutton('editauctioneer.apply');">
				<i class="icon-new"></i> <?php echo JText::_('JSAVE'); ?>
			</button>
		</div>
		<div class="btn-group">
			<button type="button" class="btn btn-primary" onclick="Joomla.submitbutton('editauctioneer.cancel');">
	...
	<div class="row-fluid">
		<div class="span10 form-horizontal">
			<fieldset>
				...
				<?php echo JHtml::_('bootstrap.addPanel', 'myTab', 'details', empty($this->item->id) ? JText::_('COM_GOTAUCTION_NEW_AUCTIONEER', true) : JText::sprintf('COM_GOTAUCTION_EDIT_AUCTIONEER', $this->item->id, true)); ?>
				
					<!-- the data rendered below comes from models/forms/designer.xml -->
					<?php foreach($this->form->getFieldset('auctioneer_fields') as $field): ?>					
						<?php 							
							$style="";							
